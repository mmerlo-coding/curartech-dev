const SingleFaq = (props: { question: string; answer: string; isAlternate?: boolean }) => {
  const { question, answer, isAlternate = false } = props;

  return (
    <>
    <div className={`mb-12 flex lg:mb-[70px] ${isAlternate ? 'bg-primary text-white' : 'bg-white text-dark'}`}>
            <div
              className={`mr-4 flex h-[50px] w-full max-w-[50px] items-center justify-center rounded-xl ${isAlternate ? 'bg-primary text-white' : 'bg-primary text-white'} sm:mr-6 sm:h-[60px] sm:max-w-[60px]`}>
            ...
            <div className="w-full p-6">
              <h3 className={`mb-6 text-xl font-semibold ${isAlternate ? 'text-white' : 'text-dark dark:text-white'} sm:text-2xl lg:text-xl xl:text-2xl`}>
                {question}
              </h3>
              <p className={`text-base ${isAlternate ? 'text-white/90' : 'text-body-color dark:text-dark-6'}`}>
                {answer}
              </p>
            </div>
          </div>
